...
Vector LevelSet::getNormalVector(array<int, 3> cell, bool useInterpolation /* = true */, bool normalizeVector /* = true */, bool findCPin2D /* = true */) const
{
    ...
    if (numZ == 1 && findCPin2D && useInterpolation)
    {
        ...
        if (trackedCP == "left")
        {
            ...
            normalX = alpha * (this->at(cell[0], cell[1], cell[2]) - this->at(cell[0] - 2, cell[1], cell[2])) / (2 * dx) + (1 - alpha) * (this->at(cell[0] + 1, cell[1], cell[2]) - this->at(cell[0] - 1, cell[1], cell[2])) / (2 * dx);
            normalY = alpha * (-this->at(cell[0] - 1, cell[1] + 2, cell[2]) + 4.0 * this->at(cell[0] - 1, cell[1] + 1, cell[2]) - 3.0 * this->at(cell[0] - 1, cell[1], cell[2])) / (2 * dy) + (1 - alpha) * (-this->at(cell[0], cell[1] + 2, cell[2]) + 4.0 * this->at(cell[0], cell[1] + 1, cell[2]) - 3.0 * this->at(cell[0], cell[1], cell[2])) / (2 * dy); // second order difference quotient
        }
        else if (trackedCP == "right")
        {
            ...
            normalX = alpha * (this->at(cell[0] + 1, 0, 0) - this->at(cell[0] - 1, 0, 0)) / (2 * dx) + (1 - alpha) * (this->at(cell[0] + 2, 0, 0) - this->at(cell[0], 0, 0)) / (2 * dx);
            normalY = alpha * (-this->at(cell[0], cell[1] + 2, cell[2]) + 4.0 * this->at(cell[0], cell[1] + 1, cell[2]) - 3.0 * this->at(cell[0], cell[1], cell[2])) / (2 * dy) + (1 - alpha) * (-this->at(cell[0] + 1, cell[1] + 2, cell[2]) + 4.0 * this->at(cell[0] + 1, cell[1] + 1, cell[2]) - 3.0 * this->at(cell[0] + 1, cell[1], cell[2])) / (2 * dy);
        }
    }
    else
    {
        ...
        {
            normalY = (this->at(cell[0], cell[1] + 1, cell[2]) - this->at(cell[0], cell[1] - 1, cell[2])) / (2 * dy);
        }
        else if (cell[1] == 0)
        {
        ...
        {
            normalY = (this->at(cell[0], cell[1] - 2, cell[2]) - 4 * this->at(cell[0], cell[1] - 1, cell[2]) + 3 * this->at(cell[0], cell[1], cell[2])) / (2 * dy);
        }
...
double LevelSet::getContactAngleInterpolated(int timestep)
{
    ...
    else
    {
        ...
        normal = (1 - mu) * ((1 - lambda) * getNormalVector(i, j, k) + lambda * getNormalVector(i + 1, j, k)) + mu * ((1 - lambda) * getNormalVector(i, j, k + 1) + lambda * getNormalVector(i + 1, j, k + 1));
    }
 ...
 * @param cell The indices of the contact point
 */
void LevelSet::referenceCurvatureExplicitEuler(double dt, int last_timestep, double initCurvature, std::string reference3D)
{
    ...
    else if (reference3D == "fieldFormulation")
    {
        ...
        for (int i = 0; i < last_timestep; i++)
        {
            ...
            if (cell[0] == 0)
            {
                dnx_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[0] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * d);

                dny_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[1] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * d);

                if (numZ > 1)
                {
                    dnz_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[2] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * d);
                }
            }
            ...
            dnx_dy = (-localField.at(local, 2, sidelengthZ / 2)[0] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * d);

            dny_dy = (-localField.at(local, 2, sidelengthZ / 2)[1] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * d);
            ...
            if (numZ > 1)
            {
                ...
                if (cell[2] == 0)
                {
                    dnx_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[0] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * d);
                    dny_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[1] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * d);
                    dnz_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[2] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * d);
                }
                ...
                dnz_dy = (-localField.at(local, 2, sidelengthZ / 2)[2] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * d);
            }
    ...
    else if (reference3D == "sectionalCurvature")
    {
        ...
        for (int i = 0; i < last_timestep; i++)
        {
            ...
            secCurvA = secCurvA + dt * (tempA * normal + secCurvA * ((field->gradAt(i * dt, CP[0], CP[1], CP[2])) * normal) * normal - 2 * secCurvA * (field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentA) * tangentA);

            Vector tempB = field->secondPartial(i * dt, CP[0], CP[1], CP[2], tangentB);
            secCurvB = secCurvB + dt * (tempB * normal + secCurvB * ((field->gradAt(i * dt, CP[0], CP[1], CP[2])) * normal) * normal - 2 * secCurvB * (field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentB) * tangentB);

            Vector tempC = field->secondPartial(i * dt, CP[0], CP[1], CP[2], tangentC);
            secCurvC = secCurvC + dt * (tempC * normal + secCurvC * ((field->gradAt(i * dt, CP[0], CP[1], CP[2])) * normal) * normal - 2 * secCurvC * (field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentC) * tangentC);
...
void LevelSet::referenceTangentExplicitEuler(double dt, int last_timestep, Vector normal_init)
{
    ...
    for (int i = 0; i < last_timestep; i++)
    {
        ...
        derivA = field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentA - ((field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentA) * tangentA) * tangentA;
        derivB = field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentB - ((field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentB) * tangentB) * tangentB;
        derivC = field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentC - ((field->gradAt(i * dt, CP[0], CP[1], CP[2]) * tangentC) * tangentC) * tangentC;
...
double LevelSet::getCurvature(array<int, 3> cell) const
{
    ...
    if (cell[0] == 0)
    {
        dnx_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[0] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * dx);

        dny_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[1] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * dx);

        if (numZ > 1)
        {
            dnz_dx = (-localField.at(local + 2, 0, sidelengthZ / 2)[2] + 4.0 * localField.at(local + 1, 0, sidelengthZ / 2)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * dx);
        }
    }
    else if (cell[0] == numX - 1)
    {
    ...
    dnx_dy = (-localField.at(local, 2, sidelengthZ / 2)[0] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * dy);

    dny_dy = (-localField.at(local, 2, sidelengthZ / 2)[1] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * dy);
    ...
    if (numZ > 1)
    {
        ...
        if (cell[2] == 0)
        {
            dnx_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[0] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[0] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * dz);
            dny_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[1] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[1] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[1]) / (2 * dz);
            dnz_dz = (-localField.at(local, 0, sidelengthZ / 2 + 2)[2] + 4.0 * localField.at(local, 0, sidelengthZ / 2 + 1)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * dz);
        }
        else if (cell[2] == numZ - 1)
        {
            dnx_dz = (localField.at(local, 0, sidelengthZ / 2 - 2)[0] - 4.0 * localField.at(local, 0, sidelengthZ / 2 - 1)[0] + 3.0 * localField.at(local, 0, sidelengthZ / 2)[0]) / (2 * dz);
        ...
        dnz_dy = (-localField.at(local, 2, sidelengthZ / 2)[2] + 4.0 * localField.at(local, 1, sidelengthZ / 2)[2] - 3.0 * localField.at(local, 0, sidelengthZ / 2)[2]) / (2 * dy);
    }
...
double LevelSet::getSectionalCurvatureInterpolated(int timestep, Vector tau)
{
    ...
    double curvature = (1 - mu) * ((1 - lambda) * getSectionalCurvature({i, j, k}, tau) + lambda * getSectionalCurvature({i + 1, j, k}, tau)) + mu * ((1 - lambda) * getSectionalCurvature({i, j, k + 1}, tau) + lambda * getSectionalCurvature({i + 1, j, k + 1}, tau));
...
double LevelSet::getCurvatureInterpolated(int timestep) const
{
    ...
    else
    {
        ...
        curvature = (1 - mu) * ((1 - lambda) * getCurvature(i, j, k) + lambda * getCurvature(i + 1, j, k)) + mu * ((1 - lambda) * getCurvature(i, j, k + 1) + lambda * getCurvature(i + 1, j, k + 1));
    }
    ...
    if (timestep == 0)
    {
        ...
        *tauXmfFile << "<?xml version=\"1.0\" ?>\n"
        ...
        std::string temp = "]>\n"
                           ...
                           std::to_string(total_writesteps) + "\">\n";
...
double LevelSet::getGradPhiNormAtContactPoint(int timestep)
{
    ...
    else
    {
        ...
        Vector gradPhi = (1 - mu) * ((1 - lambda) * getNormalVector({i, j, k}, false, false) + lambda * getNormalVector({i + 1, j, k}, false, false)) + mu * ((1 - lambda) * getNormalVector({i, j, k + 1}, false, false) + lambda * getNormalVector({i + 1, j, k + 1}, false, false));
        double gradPhiNorm = abs(gradPhi);
        return gradPhiNorm;
    }
}
